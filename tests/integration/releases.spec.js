  ...
  test('CVE action report recovers when the refresh request times out at the gateway', async ({ page }) => {
    ...
    const cachedReport = {
      ...
      summary: {
        ...
        overdue: { count: 0, jql: 'https://jira.example/overdue', byCvss: [] },
        noSlaDate: { count: 0, jql: 'https://jira.example/no-sla', byCvss: [] },
        missingOutcome: { count: 0, jql: 'https://jira.example/missing-outcome', byCvss: [] }
      },
      timeline: [],
      createdWeekly: [],
      openAgeBuckets: []
    ...
    const selector = page.locator('#cve-action-component');
    await selector.selectOption({ label: 'Model Serving' });
    await expect(page.getByText(`Upcoming SLA deadlines until ${cachedReport.windowEndDate}`)).toBeVisible();

    await page.getByRole('button', { name: 'Refresh from Jira' }).click();
    await expect(page.getByRole('button', { name: 'Refreshing...' })).toBeDisabled();
    await expect(page.getByText(`Upcoming SLA deadlines until ${cachedReport.windowEndDate}`)).toBeVisible();
    await expect(page.getByText('Upcoming SLA deadlines until 2099-12-31')).toBeVisible({ timeout: 10000 });

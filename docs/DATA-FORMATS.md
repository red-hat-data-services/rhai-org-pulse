...
The Releases module reads the sheet through Org Pulse's existing per-user Google OAuth integration. It tries the configured `AIPCC_MILESTONES_GOOGLE_USER`, the last successful reader, the requesting user, and then other connected Org Pulse Google users until one can read the sheet. A successful request records that user as the credential owner for hourly background refreshes; the resulting cached schedule remains available to all authorized viewers. Administrators can refresh on demand through `POST /api/modules/releases/aipcc-milestones/refresh`. GET requests use a 15-minute freshness window; after that window they attempt a live refresh. If Google Sheets is temporarily unavailable, the GET endpoint serves the stored copy with `cacheStatus: "stale"`.

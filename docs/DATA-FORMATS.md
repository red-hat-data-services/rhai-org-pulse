...
| `features[].fpdor.items[]` | array | v4 | Array of 9 `{ name, pass, source, state }` objects. `pass`: `true`/`false`/`null`; `source`: `"jira"` or `"strat-pipeline"`; `state`: `"passed"`, `"failed"`, or `"not-evaluated"`. |
| `features[].fpdor.passedCount` | number | v4 | Items where `pass === true`. |
| `features[].fpdor.evaluatedCount` | number | v4 | Items where `pass !== null` (6 jira items always evaluated). |
| `features[].fpdor.totalCount` | number | v4 | Always 9 (6 jira + 3 strat-pipeline). |

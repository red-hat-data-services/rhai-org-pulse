    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div class="flex items-baseline gap-1.5">
            ...
            <span class="text-xs text-gray-400 dark:text-gray-500">last 90d</span>
          </div>
          <div
          ...
          >
            <div v-for="d in stats.recentDropsDetail" :key="d.name" class="flex items-center justify-between gap-2">
              <span class="truncate text-gray-700 dark:text-gray-300">{{ d.name }}</span>
              <span class="text-gray-400 dark:text-gray-500 whitespace-nowrap">{{ shortDate(d.date) }}</span>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ stats.reachedProduction }}</div>
          <div
            v-if="hoveredCard === 'prod' && stats.prodDropsDetail.length"
            class="absolute z-10 left-0 right-0 top-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-3 text-xs space-y-1.5"
          >
            <div v-for="d in stats.prodDropsDetail" :key="d.name" class="flex items-center justify-between gap-2">
              <span class="truncate text-gray-700 dark:text-gray-300">{{ d.name }}</span>
              <span v-if="d.days != null" class="text-gray-400 dark:text-gray-500 whitespace-nowrap">{{ d.days }}d</span>
            </div>
          </div>
        </div>
      ...
      <section class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700/60 p-5">
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">
          Production artifacts showing which base images each product container was built from
        </p>

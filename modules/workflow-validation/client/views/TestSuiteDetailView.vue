<template>
  <div>
    <button class="mb-4 inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200" @click="nav.navigateTo('test-suites', { suite })">
      <ChevronLeftIcon :size="16" /> Test Runs
    </button>
    <div v-if="loading" class="py-10 text-center text-gray-400">Loading test run…</div>
    ...
    <template v-else-if="data">
      <div class="mb-5">
        ...
        <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Test run from {{ formatDate(data.timestamp) }}</p>
      </div>
      ...
      <div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-gray-700/60 dark:bg-gray-800">
        <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700/60"><h3 class="font-semibold">Tests in this Run</h3></div>
...
import { compareTableValues, displayedOutcomeCounts, displayedTestOutcome, formatBuildId, formatDate, formatPercent, formatRatio, formatSuiteName, useWorkflowValidation } from '../composables/useWorkflowValidation'

const nav = inject('moduleNav')
const { getTestSuite } = useWorkflowValidation()
const suite = ref('')
const data = ref(null)
const outcomes = computed(() => displayedOutcomeCounts(data.value?.tests || []))
...
async function load() {
  ...
  if (!suite.value || !invocationId) { error.value = 'No test run specified'; return }
  loading.value = true
  error.value = ''
  try { data.value = await getTestSuite(suite.value, invocationId) }
  catch (err) { error.value = err.data?.error || err.message || 'Failed to load test run' }

<template>
  <div class="space-y-6">
    ...
    <div v-if="showMigrateConfirm" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showMigrateConfirm = false">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">Migrate from Google Sheets?</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">
          This will create in-app teams and assignments from your current Google Sheets data. The operation is additive — it creates new entries in the team store.
        </p>
        <p class="text-sm text-amber-700 dark:text-amber-400 mb-4">
          After migration, the in-app data source becomes authoritative. Sheets data will no longer be used for team assignments.
        </p>
        <div class="flex justify-end gap-3">
          <button
            @click="showMigrateConfirm = false"

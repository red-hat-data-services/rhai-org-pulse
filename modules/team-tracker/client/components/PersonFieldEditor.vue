...
function startEdit(fieldId) {
  ...
  editValue.value = props.customFields[fieldId] || ''
}

async function saveEdit(fieldId) {
  saving.value = true
  try {
    await updatePersonFields(props.uid, { [fieldId]: editValue.value || null })
  ...
  <div class="space-y-2">
    ...
    <div v-for="field in visibleFields" :key="field.id" class="flex items-center gap-2">
      ...
      <template v-if="editingFieldId === field.id">
        <input
          v-if="field.type === 'free-text' || field.type === 'person-reference-unlinked'"
          v-model="editValue"
        ...
        <input v-else v-model="editValue" class="flex-1 rounded border-gray-300 text-sm" @keyup.enter="saveEdit(field.id)">
      ...
      <template v-else>
        <span class="text-sm text-gray-900 flex-1">{{ customFields[field.id] || '-' }}</span>
        <button
          v-if="canEdit"
    ...
    <div v-if="visibleFields.length === 0" class="text-sm text-gray-500">

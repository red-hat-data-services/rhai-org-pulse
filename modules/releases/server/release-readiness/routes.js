 ...
 *     responses:
 *       200:
 *         description: Object with versions array, releases array, and default_version (latest GA date on or before today, or null)
 */
...
function registerReleaseReadinessRoutes(router, { storage, requireAuth, requireScope }) {
  ...
  router.get('/versions', requireAuth, requireScope('releases:read'), async (req, res) => {
    ...
    try {
      ...
      return res.json({ versions: [], releases: [], default_version: null });
    }
